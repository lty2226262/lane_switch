...
class ViTExtractor:
    ...
    def preprocess_tensor(self, image_tensor: torch.Tensor, resize_shape=(224, 224)) -> torch.Tensor:
        """
        Preprocesses an image tensor before extraction.
        :param image_tensor: tensor of shape BxCxHxW to be extracted.
        :return: the preprocessed image as a tensor to insert the model of shape BxCxHxW.
        """
        prep = transforms.Compose([
            transforms.Resize(resize_shape, interpolation=transforms.InterpolationMode.BICUBIC),
            transforms.Normalize(mean=self.mean, std=self.std)
        ])
...
class GSRefinerPipeline(
        ...
        **kwargs,
        ...
        Args:
            ...
            prompt_embeds (`torch.FloatTensor`, *optional*):
                ...
                provided, text embeddings will be generated from the `prompt` input argument.
            negative_prompt_embeds (`torch.FloatTensor`, *optional*):
        ...
        assert ref_image is not None, "Please provide a reference image for guided image generation."
        ...
        cos_sim = self.chunk_cosine_sim(image, ref_image)
        ...
        measurement = latents.chunk(2, dim=0)[0]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                prev_sample = latents.clone().detach()
                all_output = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=True)
                latents = all_output.prev_sample

                if cos_sim > self.dino_threshold:
                    norm_grad, norm = self.grad_and_value(measurement,
                                                          prev_sample,
                                                          t,
                                                          noise_pred)
                    latents = latents - norm * norm_grad * 0.07
    ...
    def grad_and_value(self, measurement, cur_sample, cur_timestep, model_output):
        with torch.inference_mode(False), torch.enable_grad():
            x = cur_sample[0].unsqueeze(0).clone().requires_grad_(True)
            model_output_x = model_output[0].squeeze(0)
            measurement = measurement
            
            alpha_prod_t = self.scheduler.alphas_cumprod[cur_timestep].to(self.device)
            alpha_prod_t_prev = self.scheduler.one.unsqueeze(0).to(self.device)
            ...
            pred_original_sample = (x - beta_prod_t ** (0.5) * model_output_x) / alpha_prod_t ** (0.5)
            ...
            # 6. Add noise
            variance = 0
            device = model_output_x.device
            variance_noise = randn_tensor(
                model_output_x.shape, device=device, dtype=model_output_x.dtype
            )
            if self.scheduler.variance_type == "fixed_small_log":
                variance = self.scheduler._get_variance(cur_timestep, predicted_variance=None) * variance_noise
            elif self.scheduler.variance_type == "learned_range":
                variance = self.scheduler._get_variance(cur_timestep, predicted_variance=None)
                variance = torch.exp(0.5 * variance) * variance_noise
            else:
                variance = (self.scheduler._get_variance(cur_timestep, predicted_variance=None) ** 0.5) * variance_noise

            x0_hat = pred_prev_sample + variance
            blured_x0_hat = self.get_low_or_high_fft(x0_hat, self.fft_scale, is_low=True)
            difference = measurement - blured_x0_hat
            norm = torch.norm(difference, p=2)
            norm_grad = torch.autograd.grad(outputs=norm, inputs = x)[0]
        return norm_grad, norm

    def get_low_or_high_fft(self, x, scale, is_low=True):
        # Convert to float32 for FFT operations (cuFFT limitation with half precision and non-power-of-2 dimensions)
        original_dtype = x.dtype
        x_float = x.float()
        
        x_freq = torch.fft.fftn(x_float, dim=(-2, -1))
        x_freq = torch.fft.fftshift(x_freq, dim=(-2, -1))
        T, C, H, W = x_freq.shape

        mask = torch.zeros_like(x_freq) if is_low else torch.ones_like(x_freq)
        crow, ccol = H // 2, W // 2

        if is_low:
            mask[:, :, max(crow - int(crow * scale), 0):crow + int(crow * scale), 
                    max(0, ccol - int(ccol * scale)):ccol + int(ccol * scale)] = 1
        else:
            mask[:, :, max(crow - int(crow * scale), 0):crow + int(crow * scale), 
                    max(0, ccol - int(ccol * scale)):ccol + int(ccol * scale)] = 0
    ...
    @torch.no_grad()
    def chunk_cosine_sim(self, x: torch.Tensor, y: torch.Tensor, layer=11, facet='key', bin=False) -> torch.Tensor:
        ...
        device = x.device
        self.dino_extractor.model.to(device)
        extractor = self.dino_extractor

        result_list = []
        image_batch_a = extractor.preprocess_tensor(x * 0.5 + 0.5)
        image_batch_b = extractor.preprocess_tensor(y * 0.5 + 0.5)
        ...
        result_list = []
        num_token_x = descs_a.shape[2]
        for token_idx in range(num_token_x):
            token = descs_a[:, :, token_idx, :].unsqueeze(dim=2)
            result_list.append(torch.nn.CosineSimilarity(dim=3)(token, descs_b))

        result = torch.stack(result_list, dim=2).max(dim=-1)[0].mean()
        return result